...
The tag `xgoto` must be followed by a list of at least two elements of a structure composed by two fields: `if` and `then`. In this example we state that, once "First task" is completed, `if` the expression `${condition1}` is true, `then` we go to to task `Condition1 second task`, else `if` the expression `${condition2}` is true, `then` we go to to task `Condition2 second task`.
